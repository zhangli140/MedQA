...
num_steps = 512 # it must consist with the test
...
with tf.Session(config=config) as sess:
	with tf.device(gpu_config):
		...
		with tf.variable_scope("model", reuse=None, initializer=initializer):
			model = BILSTM_CRF(num_chars=num_chars, num_classes=num_classes, num_steps=num_steps, num_epochs=num_epochs, embedding_matrix=embedding_matrix, is_training=True, early_stopping=True, exponential_decay=True)
