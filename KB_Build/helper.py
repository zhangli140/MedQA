...
def getEmbedding(infile_path="embedding"):
	...
	row_index = 0
	with open(infile_path, "rb", encoding='utf8') as infile:
		for row in infile:
			row = row.strip()
			row_index += 1
			if row_index == 1:
...
def loadMap(token2id_filepath):
	...
	token2id = {}
	id2token = {}
	with open(token2id_filepath, encoding='gbk') as infile:
		for row in infile:
			row = row.rstrip()
			token = row.split('\t')[0]
...
def saveMap(id2char, id2label):
	with open("char2id", "w", encoding='utf8') as outfile:
		for idx in id2char:
			outfile.write(id2char[idx] + "\t" + str(idx)  + "\n")
	with open("label2id", "w", encoding='utf8') as outfile:
		for idx in id2label:
